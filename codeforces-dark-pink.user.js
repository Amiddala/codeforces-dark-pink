...
(function () {
  ...
  (function fixGreenContrast() {
    if (document.readyState !== "complete")
      return setTimeout(fixGreenContrast, 100);
    ...
    document.querySelectorAll("font").forEach((el) => {
      if (el.getAttribute("color") === "red")
        el.setAttribute("color", "#ff80ab");
      if (el.getAttribute("color") === "green")
        el.setAttribute("color", "#a5d6a7");
    });
  })();
  ...
  (function fixInlineWhiteBGs() {
    if (document.readyState !== "complete")
      return setTimeout(fixInlineWhiteBGs, 100);
    const whiteBGs = [
      "rgb(255, 255, 255)",
      "rgb(240, 240, 240)",
      "rgb(250, 250, 250)",
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (
        bg &&
        (whiteBGs.includes(bg) || bg.startsWith("rgb(25") || bg === "white")
      ) {
  ...
  (function fixTestCaseLines() {
    ...
    function applyBG(elm) {
      ...
      if (
        !elm.style.cssText.includes("(30") &&
        !elm.style.cssText.includes("#1e")
      )
        elm.style.cssText = "background-color: rgb(30, 0, 24) !important;";
    }
    const obs = new MutationObserver((list) =>
      list.forEach((m) => {
        if (m.target.classList?.contains(CLS)) applyBG(m.target);
      }),
    );
    waitFor("." + CLS, () =>
  ...
  waitFor(".second-level-menu-list li.backLava", (elm) => {
    elm.style.backgroundImage =
      "url(https://raw.githubusercontent.com/Amiddala/codeforces-dark-pink/main/imgs/lava-right2.png)";
    if (elm.firstElementChild)
      elm.firstElementChild.style.backgroundImage =
        "url(https://raw.githubusercontent.com/Amiddala/codeforces-dark-pink/main/imgs/lava-left2.png)";
  });
